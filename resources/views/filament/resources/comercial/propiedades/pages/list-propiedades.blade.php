<x-filament-panels::page>
    ...
    <div style="display:flex; flex-wrap:wrap; align-items:flex-end; justify-content:space-between; gap:12px; margin-bottom:24px;">
        ...
        <div style="display:inline-flex; border-radius:8px; border:1px solid #e5e7eb; overflow:hidden; box-shadow:0 1px 2px rgba(0,0,0,.05);">
            <button
            ...
            >
                <svg style="width:16px;height:16px;flex-shrink:0;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M3 6h18M3 14h18M3 18h18"/></svg>
                Lista
            </button>
            <button
                wire:click="cambiarVista('cards')"
        ...
        @else
            <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(260px, 1fr)); gap:20px;">
                @foreach($propiedades as $propiedad)
                    @include('filament.resources.comercial.propiedades.partials.propiedad-card', ['propiedad' => $propiedad])
                @endforeach
            </div>
    ...
    @if($this->vista === 'mapa')
        ...
        <div
            ...
            x-data="mapaInmuebles"
            x-init="inicializar(@js($marcadores))"
            ...
            <div style="padding:12px 16px; border-bottom:1px solid #e5e7eb; display:flex; align-items:center; justify-content:space-between;">
                ...
                @if(count($marcadores) === 0)
                    <span style="font-size:12px; color:#d97706;">
                        ⚠️ Ejecuta <code style="background:#f3f4f6; padding:2px 6px; border-radius:4px; font-family:monospace;">php artisan propiedades:geocodificar</code> para poblar coordenadas
                    </span>
                @endif
            </div>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('mapaInmuebles', () => ({
                ...
                agregarPin(prop) {
                    ...
                    const marker = L.marker([prop.lat, prop.lng], { icon: icono })
