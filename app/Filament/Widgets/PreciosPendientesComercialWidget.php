...
class PreciosPendientesComercialWidget extends BaseWidget
{
    ...
    public static function canView(): bool
    {
        ...
        return $user->can('precios_aprobar_comercial');
    }

    public function table(Table $table): Table
    {
        return $table
            ->heading('💰 Precios pendientes de aprobación (Comercial)')
            ->description('Estas propiedades requieren tu validación del precio')
            ->query(
                Propiedad::query()
                    ->whereHas('aprobacionesPrecio', function ($query) {
                        $query->where('tipo_aprobador', 'COMERCIAL')
